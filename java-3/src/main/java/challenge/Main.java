...
public class Main {
	...
	public int q2() {
		...
		return (int) listaJogadores.stream().filter(Jogador -> !Jogador.getClub().isEmpty()).map(Jogador::getClub)
				.distinct().count();
	}
	...
	public List<String> q4() {
		...
		return listaJogadores
				.stream().filter(Jogador -> !Jogador.getEur_release_clause().isEmpty()).sorted(Comparator
						.comparingDouble((Jogador j) -> Double.parseDouble(j.getEur_release_clause())).reversed())
	...
	public List<String> q5() {
		...
		return listaJogadores.stream()
				.sorted(Comparator.comparing(Jogador::getBirth_date).thenComparing(Jogador::getEur_wage))
				.map(Jogador::getFull_name).limit(10).collect(toList());
	...
	public Map<Integer, Integer> q6() {
		...
		Map<Integer, Long> mapaIdade = listaJogadores.stream()
				.collect(groupingBy((Jogador j) -> Integer.parseInt(j.getAge()), Collectors.counting()));

		Map<Integer, Integer> result = mapaIdade.entrySet().stream()
